...
Программулина на Python 3.4 которая проверяет через GoodGame API статус стрима. Предназначена через звуки оповещать пользователя о стриме людей на которых Вы постоянно опаздываете потому что не замечаете визуальные способы оповещения, как например попапы или емейлы.
...
На данной версии, программу предлагается положить в автозапуск и сворачивать, так как она проста как пень, каждую минуту опрашивая ГудГейм и издавая соответствующий звук если стрим запущен. По началу стрима программу можно спокойно закрывать.
